...
<div id="mainRight">
	...
	<c:forEach items="${items}" var="item">
		<div class="mainRightItem">
			<span> <a href='<c:url value="/item/${item.id}" />'> <img
					class="mainPictureLocation"
					src='<c:url value="/resources/upload/mainpic/${item.mainPictureLocation}" />' />
			</a>
			</span> <span class="itemsDetail"> <a
				href='<c:url value="/item/${item.id}" />'>${item.title}</a>
				<table>
					<tr>
						<th>Current Bid:</th>
